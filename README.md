...
- 点进任意邮箱看它的收件箱;点开某封邮件才算已读,未读徽标随之减一
...
- 邮件详情:纯文本 + HTML 降级显示
- 删除单封:长按菜单或侧滑触发,可选仅本地删或本地+云端都删;复制地址;mailbox token 展示供脚本使用
- 本地缓存:邮件落在本机 Room 数据库,离线也能翻历史;云端收满自动清空不影响已缓存的邮件
- 刷新邮箱(云端清空续期)同样不影响本地缓存
- 侧滑操作:右滑归档、左滑删除(删除弹确认框,不误删);长按出归档/删除菜单

**归档页**
- 收件箱右上角进入,归档的邮件单独存放,卡片上有归档数徽标
- 侧滑方向与收件箱互换:左滑取消归档(回收件箱)、右滑删除;长按同样支持
...
测试:`./gradlew testDebugUnitTest`(创建命名规则、增量同步计划、MIME 清洗、OTP 提取、JSON 往返序列化均有单测)。
